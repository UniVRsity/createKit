...
export default class HelloWorld {
	...
	private controllerPos: MRE.Vector3 = new MRE.Vector3(4, -1, 0);
	private controllerScale: MRE.Vector3 = new MRE.Vector3(2, 2, 2);
	private controllerRot: MRE.Quaternion = MRE.Quaternion.RotationAxis(MRE.Vector3.Up(),
		-180.0 * MRE.DegreesToRadians);


	private controller: MRE.Actor = null;
	...
	private async started() {
		...
		this.start = MRE.Actor.CreateFromGltf(this.assets, {
			// at the given URL
			uri: `${this.baseUrl}/altspace-cube.glb`,
			// and spawn box colliders around the meshes.
			colliderType: 'box',
		...
		startButtonBehavior.onClick(user => {
			if (!isOn && user.id === MRE.parseGuid("63be2f42-0964-38ae-ad30-e91418624dce")) {
				this.controller = this.createKit("Controller Demonstration > 1447318670182187876 Full Demoo",
					"artifact:1450169649340613008",
					this.controllerPos, this.controllerScale, this.controllerRot);
				isOn = true;
			}
			else if (isOn && user.id === MRE.parseGuid("63be2f42-0964-38ae-ad30-e91418624dce")){
